...
(defmacro nil! (var) ; THis definition tells lisp: "Whenever you see an expression of the form (nil! var), turn it into one of the form (setq var nil) before evaluating it"
  (list 'setq var nil))
                                        ...
                                        ;Many of the difficulties you might encounter  with macros can be avoided by maintaining a sharp distinction between macroexpansion step deals with expressions, and the evaluation step deals with their values.

                                        ;'When writing macros, know which computations are performed during macroexpansion, and which during evaluation, for the two steps generally operate on objects of two differente sorts. The macfoexpansion step delas with expressions, and the evaluation step deals with their values.

                                        ;Many languages offer some form of macro, buy Lisp macros are singularly powerful. When a file of Lisp is compiled, a perser reads the sourc code and sends its output to the compiler. Here's the stroke of genius: the output of the parser consists of lisp objects. With macros, we can manipulate the program while it's in this intermediate form between parser and compiler. If necessaery, these manipulations can be very extensive. A macro generating its expansion
;Oratio pro Cicerone contra Erasmum 
